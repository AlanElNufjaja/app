...
tamano_inicial = (mete['estimated_diameter.kilometers.estimated_diameter_min'] + mete['estimated_diameter.kilometers.estimated_diameter_max']) / 2 * 1000  # km a m
...
tamano_inicial = st.sidebar.slider("Tamaño del meteorito (m)", 0.1, 500.0, float(tamano))
...
radio_km = calcular_radio_impacto(tamano_inicial, densidad, velocidad_kms)

# Generar puntos de impacto (centro y radio)
df = generar_puntos_circulo(lat, lon, radio_km)

# Mostrar info
st.write(f"Tamaño: {tamano_final:.1f} m")
